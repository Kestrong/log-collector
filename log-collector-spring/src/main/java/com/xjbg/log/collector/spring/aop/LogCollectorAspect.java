...
@Aspect
@Setter
public class LogCollectorAspect implements MethodInterceptor {
        ...
        try {
            ...
            if (parameterNames != null && parameterNames.length > 0) {
                for (int i = 0, n = parameterNames.length; i < n; i++) {
                    if (getAnnotation(parameterAnnotations[i], CollectorLog.Ignore.class) != null) {
                        continue;
                    }
                    ...
                    if (parameter instanceof MultipartFile) {
                        ...
                        CollectorLog.Sensitive sensitive = getAnnotation(parameterAnnotations[i], CollectorLog.Sensitive.class);
                        if (parameter instanceof String && sensitive != null) {
            ...
            if (!collectorLog.onError() || fail) {
                try {
                    ...
                    logRequest.clear();
                    logInfoBuilder.businessNo(getSpelValue(context, collectorLog.businessNo()))
                    ...
                    LogInfo logInfo = logInfoBuilder.userAgent(LogSpringHttpRequestUtil.getUserAgent())
                            ...
                            .state(fail ? LogState.FAIL.name() : LogState.SUCCESS.name()).response(toJson(response)).responseTime(end).build();
    ...
    public Object invoke(MethodInvocation invocation) throws Throwable {
        ...
        Map<String, Object> logRequest = new HashMap<>(parameters.length);
        LogInfo.LogInfoBuilder logInfoBuilder = LogInfo.builder();
        try {
            ...
            logInfoBuilder.requestTime(begin)
                    .handleMethod(invocation.getThis().getClass().getName() + "#" + invocation.getMethod().getName())
            ...
            try {
                LogInfo logInfo = logInfoBuilder.state(fail ? LogState.FAIL.name() : LogState.SUCCESS.name())
                        .response(toJson(response)).responseTime(end).build();
    ...
    private CollectorLog getCollectorLogAnnotation(Method method) {
    ...
    private <T extends Annotation> T getAnnotation(Annotation[] annotations, Class<T> target) {
